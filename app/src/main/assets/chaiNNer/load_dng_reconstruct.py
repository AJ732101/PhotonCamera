...
def _read_dng(path: Path) -> np.ndarray | None:
    ...
    try:
        ...
        with rawpy.imread(str(path)) as raw:
            ...
            rgb = raw.postprocess(use_camera_wb=True, bright=1.0, output_bps=16, no_auto_bright=True, highlight_mode=3)
...
@io_group.register(
    schema_id="chainner:image:load_dng_reconstruct",
    name="Load DNG (HL Reconstruct)",
    description="DNG image loader with highlight reconstruction",
...
def load_dng_reconstruct_node(path: Path) -> tuple[np.ndarray, Path, str]:
    path_obj = Path(path)
